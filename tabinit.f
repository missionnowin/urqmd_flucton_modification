...
c     loop over first out-particle N & D
      do 81 i1=1,maxdel
cJS      do 81 ii1=1,2
cJS         if(ii1.eq.1)i1=minnuc
cJS         if(ii1.eq.2)i1=mindel
c     loop over second out-particle N(1440)..maxdel
         do 82 i2=minnuc+1,maxdel
c     loop over all x-values
            do 83 i=1,widnsp
c     store the values ...
               frrtaby(i,1,i1,i2)=fppfit(99,tabxnd(i),i1,i2)
 83         continue
c     calculate the second derivate and store it in 'frrtaby(,,2)'
            call spline (tabxnd(1),frrtaby(1,1,i1,i2),widnsp,abl0,abln,
     .           frrtaby(1,2,i1,i2))
 82      continue
 81   continue

c..   really all pp-exit channels
c     loop over first out-particle N* and up 
c      do 81 i1=2,maxdel
c         if(ii1.eq.1)i1=minnuc
c         if(ii1.eq.2)i1=mindel
c     loop over second out-particle N(1440)..maxdel
c         do 82 i2=2,maxdel
c     loop over all x-values
c            do 83 i=1,widnsp
c     store the values ...
c               frrtaby(i,1,ii1,i2)=fppfit(99,tabxnd(i),i1,i2)
c               if((i1.eq.2.and.i2.eq.17).or.(i1.eq.17.and.i2.eq.2))then
c                  frrtaby(i,1,ii1,i2)=0.0d0 
c               endif
c 83         continue
c     calculate the second derivate and store it in 'frrtaby(,,2)'
c            call spline (tabxnd(1),frrtaby(1,1,ii1,i2),widnsp,abl0,abln,
c     .           frrtaby(1,2,ii1,i2))
c 82      continue
c 81   continue
