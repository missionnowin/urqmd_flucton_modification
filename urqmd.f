...
ctr Init resonance reconstruction (only f13)      
...
c     exit collision loop if no collisions are left
            if (k.eq.0) goto 102
c     hp call hydro if start time is reached
            if(CTOption(45).eq.1)then         
...
c     hp all particle arrays will be modified by hydro
...
c     hp full update of collision table
                     call colload
