...
cJS    this can be used to select events with certain N_part
cccccccccccccccccccccccccccccc
c      do i=1,npart
c         if(ncoll(i).eq.0.and.ityp(i).eq.1)then
c            numspechyd=numspechyd+1
c         endif
c      enddo

c      if(numspechyd.lt.0.or.numspechyd.gt.5)then
c         return
c      else
c         write(15,*)numspechyd
c      endif
...
cJS  calculate volume of hypersurface
...
c        vyb=vyfr(m)
c        vzb=vzfr(m)
c        beta2=vxb**2+vyb**2+vzb**2
c        gam=1.d0/(sqrt(1.d0-beta2))
c        dst=dstfr(m)
c        dsx=dsxfr(m)
c        dsy=dsyfr(m)
c        dsz=dszfr(m)
c      voltot=voltot+((1/hqc)**3)*gam*(dst+dsx*vxb+dsy*vyb+dsz*vzb)
c      enddo     
...
c..   Juettner distribution for all other particle species         
        else 
           pnd(ii)=volume*degfac
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 savr0(nmax), savrx(nmax), savry(nmax), savrz(nmax),
     +     savp0(nmax), savpx(nmax), savpy(nmax), savpz(nmax),
     +     savfmass(nmax), savtform(nmax),savxtotfac(nmax),
     $     savdectime(nmax)
      integer savncoll(nmax),savcharge(nmax),savityp(nmax),
     &     savlstcoll(nmax),saviso3(nmax),savorigin(nmax)
      common /savcoor/savr0,savrx,savry,savrz,savp0,savpx, 
     &     savpy,savpz,savfmass,savtform,savxtotfac,
     &     savncoll,savcharge,savityp,savlstcoll,saviso3,savorigin
     $     ,savdectime

      real*8 savffermpx(nmax), savffermpy(nmax), savffermpz(nmax)
      integer savinew(nmax),savspin(nmax),savuid(nmax)
      common /savffermi/ savffermpx, savffermpy, savffermpz

      real*8 savfrr0(nmax), savfrrx(nmax), savfrry(nmax),
     &       savfrrz(nmax),savfrp0(nmax), savfrpx(nmax), 
     &       savfrpy(nmax), savfrpz(nmax)

      common /savfrcoor/ savfrr0, savfrrx, savfrry, savfrrz,
     &                    savfrp0, savfrpx, savfrpy, savfrpz
      

      integer numsav,CTOsave,stidx,nexitsav,ctagsav,dectagsav
     $     ,nmessav,nbarsav
      common /savn/ numsav,nexitsav,savinew,ctagsav,dectagsav
     $     ,savspin,savuid,nmessav,nbarsav
      integer i
      logical isstable
      real*8 xdummy
...
c     Save all particle Lists
      ...
      do i=1,npart
         ...
         savityp(i)=ityp(i)
         saviso3(i)=iso3(i)
         savcharge(i)=charge(i)
         savlstcoll(i)=lstcoll(i)
         savncoll(i)=ncoll(i)
         savorigin(i)=origin(i)
         savdectime(i)=dectime(i)
         savtform(i)=tform(i)
         savxtotfac(i)=xtotfac(i)
         savspin(i)=spin(i)
         savuid(i)=uid(i)
         savfrr0(i) = frr0(i)
         savfrrx(i) = frrx(i)
         ...
         savfrpy(i) = frpy(i)
         savfrpz(i) = frpz(i)
         savffermpx(i)=ffermpx(i)
         savffermpy(i)=ffermpy(i)
         savffermpz(i)=ffermpz(i)      
      enddo
...
c      write(*,*) "save and dec ", npart,nmes,nbar
c     now do the decays
c     no do-loop is used because npart changes in loop-structure
      i=0
c     disable Pauli-Blocker for final decays
...
c     perform decay
            call scatter(i,0,0.d0,fmass(i),xdummy)
c     backtracing if decay-product is unstable itself
            if(dectime(i).lt.1.d30) goto 416
         endif
      endif
c     check next particle
      if(i.lt.npart) goto 406
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &                    savfrp0, savfrpx, savfrpy, savfrpz
      ...
      integer numsav,nexitsav,savinew(nmax),ctagsav,dectagsav
     $     ,nmessav,nbarsav
      common /savn/ numsav,nexitsav,savinew,ctagsav,dectagsav
     $     ,savspin,savuid,nmessav,nbarsav
      integer i
...
c     write(*,*)"restore", npart,nmes,nbar
      ...
      do i=1,npart
         ...
         ityp(i)=savityp(i)
         iso3(i)=saviso3(i)
         charge(i)=savcharge(i)
         lstcoll(i)=savlstcoll(i)
         ncoll(i)=savncoll(i)
         origin(i)=savorigin(i)
         dectime(i)=savdectime(i)
         tform(i)=savtform(i)
         xtotfac(i)=savxtotfac(i)
         spin(i)=savspin(i)
         uid(i)=savuid(i)
         frr0(i)=savfrr0(i)
         frrx(i)=savfrrx(i)
         ...
         frpy(i)=savfrpy(i)
         frpz(i)=savfrpz(i)
         ffermpx(i)=savffermpx(i)
         ffermpy(i)=savffermpy(i)
         ffermpz(i)=savffermpz(i)
