...
c     we call the translation routine
      ...
      goto 1
c     spl... string fragmentation called with quark id's and energy as arguments
...
c udc
       if(chrmres(itypabs).eq.1.and.t3.eq.0)then
          if(1)=1
          if(2)=2
          if(3)=4
        call mquarks(if,ifa,ifb)
...
c       if(ityp.eq.-70)then
c          if(1)=-1
c          if(2)=-2
c          if(3)=-4
c        call mquarks(if,ifa,ifb)
c        ifa=ifa*isign(1,ityp)
c        ifb=ifb*isign(1,ityp)
c        return
c       endif
...
c cascade
               ...
               IT=isign(iit,id)
               return
            else if(idabs.eq.1240.or.idabs.eq.2140)then
c     Lambda_c
               iit=minlamc
               IT=isign(iit,id)
               return
